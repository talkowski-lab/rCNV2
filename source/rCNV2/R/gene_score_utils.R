...
#' @param hc.cutoff minimum score for high-confidence designation \[default: 0.9\]
#' @param lc.cutoff minimum score for low-confidence designatnion \[default: 0.5\]
#'
...
#' @export
classify.genes.by.score <- function(scores, hc.cutoff=0.9, lc.cutoff=0.5){
  ds.hc <- scores$gene[which(scores$pHaplo>=hc.cutoff & scores$pTriplo>=hc.cutoff)]
  ds.lc <- scores$gene[which(scores$pHaplo>=lc.cutoff & scores$pTriplo>=lc.cutoff & !(scores$gene %in% ds.hc))]
  hi.hc <- scores$gene[which(scores$pHaplo>=hc.cutoff & scores$pTriplo<lc.cutoff)]
  hi.lc <- scores$gene[which(scores$pHaplo>=lc.cutoff & scores$pTriplo<lc.cutoff & !(scores$gene %in% hi.hc))]
  ts.hc <- scores$gene[which(scores$pHaplo<lc.cutoff & scores$pTriplo>=hc.cutoff)]
  ts.lc <- scores$gene[which(scores$pHaplo<lc.cutoff & scores$pTriplo>=lc.cutoff & !(scores$gene %in% ts.hc))]
  ns <- scores$gene[which(scores$pHaplo<lc.cutoff & scores$pTriplo<lc.cutoff)]
