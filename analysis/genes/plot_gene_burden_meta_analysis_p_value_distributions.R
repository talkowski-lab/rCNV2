...
# Plot distributions of primary & secondary p-values from gene association meta analysis

# DEV NOTE: this code snippet is mostly saved for convenience, and references paths
#           to local files on RLC's computer that were generated by gene_burden_analysis.sh


options(scipen=1000, stringsAsFactors=F)


# Plot histogram of observed primary p-values from rCNV sliding gene meta-analysis

x <- read.table("~/scratch/rCNV.observed_pval_matrix.txt.gz", header=T, sep="\t")
x <- t(apply(x, 1, function(x){10^-as.numeric(x)}))

pdf("~/scratch/rCNV.observed_pval_distribution.gene_burden_meta_analysis.pdf",
    height=5, width=8)
hist(x[which(x<1)], breaks=200, col="gray20", freq=F,
     xlab="Meta-analysis P-value", 
     main=paste("Distribution of meta-analysis P-values",
                "across all phenotypes, gene, and CNV classes",
                sep="\n"))
mtext(3, line=-1.5, text=paste("Note: genes with P=1 not shown (i.e., where zero case CNVs are observed).", 
                               "These comprise the majority of all genes.", sep="\n"),
      font=3, cex=0.85)
dev.off()


# Plot histogram of observed secondary p-values from rCNV sliding gene meta-analysis

s <- read.table("~/scratch/rCNV.observed_pval_matrix_secondary.txt.gz", header=T, sep="\t")
s <- t(apply(s, 1, function(s){10^-as.numeric(s)}))

pdf("~/scratch/rCNV.observed_secondary_pval_distribution.gene_burden_meta_analysis.pdf",
    height=5, width=8)
hist(s[which(s<1)], breaks=200, col="gray20", freq=F,
     ...
     main=paste("Distribution of secondary meta-analysis P-values",
                "across all phenotypes, genes, and CNV classes",
                sep="\n"))
mtext(3, line=-1.5, text=paste("Note: genes with P=1 not shown (i.e., where zero case CNVs are observed).", 
                               "These comprise the majority of all genes.", sep="\n"),
      font=3, cex=0.85)
...
png("~/scratch/rCNV.gene_burden.primary_vs_secondary_pvalues.png", height=300*5, width=300*5, res=300)
...
bins <- read.table("~/scratch/rCNV.gene_burden.meta_analysis.bins.bed.gz",
