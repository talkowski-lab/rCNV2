...
# Extract significant genes from a p-value matrix
...
load.bed <- function(infile, skip.cols=1:4){
  x <- read.table(infile, header=T, comment.char="", sep="\t")
  colnames(x)[1] <- "chr"
  x[, -skip.cols] <- apply(x[, -skip.cols], 2, as.numeric)
  return(x)
}


# Load p-value matrix
load.pvalues <- function(pvalues.in, bed, p.is.neg.log10, skip.cols=1:4){
  pvalues <- load.bed(pvalues.in)
  if(p.is.neg.log10==T){
    pvalues[, -skip.cols] <- apply(pvalues[, -skip.cols], 2, function(p){10^-p})
  }
  pvalues <- merge(pvalues, bed, all.x=F, all.y=T, 
                   by=c("chr", "start", "end", "gene"), sort=F)
  return(pvalues)
}
...
load.ors <- function(ors.in, bed, or.is.ln, skip.cols=1:4){
  ors <- load.bed(ors.in)
  if(or.is.ln==T){
    ors[, -skip.cols] <- apply(ors[, -skip.cols], 2, exp)
  }
...
get.sig.pval.idxs <- function(pvalues, p.cutoffs, skip.cols=1:4){
  vals <- lapply((max(skip.cols)+1):ncol(pvalues), function(i){
    hpo <- unlist(strsplit(colnames(pvalues)[i], split=".", fixed=T))[1]
    max.p <- p.cutoffs$max.p[which(p.cutoffs$hpo == hpo)]
    which(pvalues[, i] <= max.p)
  })
  names(vals) <- colnames(pvalues)[-skip.cols]
  return(vals)
}


# Identify significant genes based on a numeric matrix
get.sig.idxs <- function(x, cutoff, direction, skip.cols=1:4){
  lapply((max(skip.cols)+1):ncol(x), function(i){
    vals <- x[, i]
    if(direction=="ge"){
      which(vals>=cutoff)
      ...
      stop(paste("get.sig.idxs: direction", direction, "is not recognized."))
    }
  })
}


# Merge lists of significant gene indexes
...
mark.sig.genes <- function(bed, sig.merged){
...
option_list <- list(
  make_option(c("--pvalues"), default=NULL, 
              help="matrix of p-values per gene per phenotype. [default %default]"),
  make_option(c("--secondary-pvalues"), default=NULL, 
              help="matrix of secondary p-values per gene per phenotype. [default %default]"),
  ...
  make_option(c("--odds-ratios"), default=NULL, 
              help="matrix of odds ratios (or lower bounds) per gene per phenotype. [default %default]"),
  ...
  make_option(c("--nominal-counts"), default=NULL, 
              help="matrix of nominally significant cohorts per gene per phenotype. [default %default]"),
  ...
  make_option(c("--secondary-or-nom"), type="logical", default=F, action="store_true",
              help="include genes that meet either secondary P-value or min. nominal significant cohort count. [default %default]"),
  make_option(c("-o", "--out-prefix"), type="character", default="./significant_genes.",
              help="prefix for writing out all results. [default %default]", metavar="path")
)

# Get command-line arguments & options
args <- parse_args(OptionParser(usage="%prog genes.bed",
...
if(length(args$args) != 1){
  stop("Must supply unannotated BED file of genes as only positional argument.\n")
}
...
# # DEV PARAMETERS:
# bed.in <- "~/scratch/all_genes.bed.gz"
# pvalues.in <- "~/scratch/DEL.pval_matrix.bed.gz"
# secondary.pvalues.in <- "~/scratch/DEL.secondary_pval_matrix.bed.gz"
# p.cutoffs.in <- "~/scratch/gene_burden.rCNV.DEL.empirical_genome_wide_pval.hpo_cutoffs.tsv"
# p.is.neg.log10 <- T
...
# min.nom <- 2
# secondary.or.nom <- T
# out.prefix <- "~/scratch/sig_genes.test."

# Read genes
bed <- load.bed(bed.in)

# Load p-values and determine significant genes, if provided
...
# Load secondary p-values and determine significant genes, if provided
if(!is.null(pvalues.in)){
...
# Load odds ratios and determine significant genes, if provided
if(!is.null(ors.in)){
  if(or.is.ln==TRUE){
    min.or <- log(min.or)
  }
...
# Load count of nominally significant cohorts and determine significant genes, if provided
...
# Determine final significant genes
sig.merged <- merge.sig(bed, sig.pvals, sig.secondary.pvals, sig.ors, sig.nom, secondary.or.nom)
bed.sigAnno <- mark.sig.genes(bed, sig.merged)
bed.sigOnly <- bed[apply(bed.sigAnno[, -c(1:4)], 1, function(vals){any(vals)}), 1:4]

# Write out
colnames(bed.sigAnno)[1] <- "#chr"
write.table(bed.sigAnno, paste(out.prefix, "all_genes_labeled.bed", sep=""), 
            sep="\t", quote=F, row.names=F, col.names=T)
colnames(bed.sigOnly)[1] <- "#chr"
write.table(bed.sigOnly, paste(out.prefix, "significant_genes.bed", sep=""), 
            sep="\t", quote=F, row.names=F, col.names=T)
