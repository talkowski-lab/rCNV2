...
# Plot CBLN2 deletion rCNV locus highlight for rCNV2 paper
...
chrom <- 18
highlight.start <- 69680000
highlight.end <- 70280000
...
# Prep locus plot
pdf(paste(out.prefix, "locus_highlight.CBLN2.pdf", sep="."), height=3*2, width=3.25*2)
...
# Add all genes
add.genes(genes[which(genes$gene != "CBLN2"), ], n.rows=1, 
...
add.genes(genes[which(genes$gene == "CBLN2"), ], n.rows=1, 
          ...
          y.axis.title="", y.axis.title.col="black", label.genes="CBLN2")
