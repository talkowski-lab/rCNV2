...
# Plot ANKRD11 rCNV locus highlight for rCNV2 paper
...
# sumstats.in <- "~/scratch/HP0001507.rCNV.DUP.sliding_window.meta_analysis.stats.bed.gz"
...
chrom <- 16
highlight.start <- 89070000
highlight.end <- 89710000
highlight.width <- highlight.end - highlight.start
viewpoint.buffer <- 0.25 * highlight.width
...
highlight.case.hpo <- "HP:0001507"
credset.genes <- c("AC137932.1", "ACSF3", "ANKRD11", "SLC22A31", "ZNF778")
...
# Prep locus plot
pdf(paste(out.prefix, "locus_highlight.ANKRD11.pdf", sep="."), height=3.75*2, width=3.25*2)
...
causal.start <- min(genes$start[which(genes$gene=="ANKRD11")], na.rm=T)
causal.end <- max(genes$end[which(genes$gene=="ANKRD11")], na.rm=T)
...
add.pvalues(ss, y0=pval.panel.y0, panel.height=pval.panel.height, cnv.type=cnv.type, 
            gw.sig=gw.sig, gw.sig.label="Genome-wide signif.")
...
# Add PIPs
add.pips(pips, genes, y0=pip.panel.y0, panel.height=pip.panel.height, label.genes=c("ANKRD11"),
...
add.genes(genes[which(genes$gene %in% credset.genes), ], n.rows=1, 
          ...
          y.axis.title="Credible Set", label.genes=c("ANKRD11"))
add.genes(genes[which(!(genes$gene %in% credset.genes)), ], n.rows=1, 
...
topbottoms <- c("bottom", "top", "bottom", "top")
