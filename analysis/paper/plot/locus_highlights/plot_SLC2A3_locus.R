...
# Plot SLC2A3 deletion rCNV locus highlight for rCNV2 paper
...
# sumstats.in <- "~/scratch/HP0001250.rCNV.DEL.sliding_window.meta_analysis.stats.bed.gz"
...
chrom <- 12
highlight.start <- 7910000
highlight.end <- 8190000
highlight.width <- highlight.end - highlight.start
viewpoint.buffer <- (1/4) * highlight.width
...
highlight.case.hpo <- "HP:0001250"
credset.genes <- c("SLC2A3")
...
# Prep locus plot
pdf(paste(out.prefix, "locus_highlight.SLC2A3.pdf", sep="."), height=3.75*2, width=3.25*2)
...
causal.start <- min(genes$start[which(genes$gene=="SLC2A3")], na.rm=T)
causal.end <- max(genes$end[which(genes$gene=="SLC2A3")], na.rm=T)
...
add.na.rect(xleft=ss.na.right.start, xright=par("usr")[2],
            y0=pval.panel.y0, panel.height=pval.panel.height,
            text="Region Excluded\nDue to Repeat Content")
add.ors(ss, y0=or.panel.y0, panel.height=or.panel.height, cnv.type=cnv.type)
add.na.rect(xleft=par("usr")[1], xright=ss.na.left.stop,
            y0=or.panel.y0, panel.height=or.panel.height,
            text="Region Excluded\nDue to Repeat Content")
add.na.rect(xleft=ss.na.right.start, xright=par("usr")[2],
            y0=or.panel.y0, panel.height=or.panel.height,
            text="Region Excluded\nDue to Repeat Content")
...
# Add PIPs
add.pips(pips, genes, y0=pip.panel.y0, panel.height=pip.panel.height, label.genes=c("SLC2A3"),
...
add.genes(genes[which(genes$gene %in% credset.genes), ], n.rows=1, 
          ...
          y.axis.title="Credible Set", label.genes=c("SLC2A3"))
add.genes(genes[which(!(genes$gene %in% credset.genes)), ], n.rows=1, 
