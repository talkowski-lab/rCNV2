...
# Plot QKI deletion rCNV locus highlight for rCNV2 paper
...
# sumstats.in <- "~/scratch/UNKNOWN.rCNV.DEL.sliding_window.meta_analysis.stats.bed.gz"
...
causal.start <- min(genes$start[which(genes$gene=="QKI")], na.rm=T)
causal.end <- max(genes$end[which(genes$gene=="QKI")], na.rm=T)
