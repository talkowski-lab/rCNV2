...
# Plot IER5 deletion rCNV locus highlight for rCNV2 paper
...
chrom <- 1
highlight.start <- 180950000
highlight.end <- 181270000
...
# Prep locus plot
pdf(paste(out.prefix, "locus_highlight.IER5.pdf", sep="."), height=3*2, width=3.25*2)
...
# Add all genes
add.genes(genes[which(genes$gene != "IER5"), ], n.rows=1, 
...
add.genes(genes[which(genes$gene == "IER5"), ], n.rows=1, 
          ...
          y.axis.title="", y.axis.title.col="black", label.genes="IER5")
