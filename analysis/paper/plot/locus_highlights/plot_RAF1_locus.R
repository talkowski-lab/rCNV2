...
# Plot RAF1 duplication rCNV locus highlight for rCNV2 paper
...
# sumstats.in <- "~/scratch/UNKNOWN.rCNV.DUP.sliding_window.meta_analysis.stats.bed.gz"
...
chrom <- 3
highlight.start <- 12550000
highlight.end <- 12880000
highlight.width <- highlight.end - highlight.start
viewpoint.buffer <- (2/3) * highlight.width
...
highlight.case.hpo <- "UNKNOWN"
credset.genes <- c("RAF1")
...
# Prep locus plot
pdf(paste(out.prefix, "locus_highlight.RAF1.pdf", sep="."), height=3*2, width=3.25*2)
...
# Add all genes
add.genes(genes[which(genes$gene != "RAF1"), ], n.rows=1, 
...
add.genes(genes[which(genes$gene == "RAF1"), ], n.rows=1, 
          ...
          y.axis.title="", y.axis.title.col="black", label.genes="RAF1")
