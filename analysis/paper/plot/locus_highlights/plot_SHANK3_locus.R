...
# Plot SHANK3/22qter deletion rCNV locus highlight for rCNV2 paper
...
chrom <- 22
highlight.start <- 50166930
highlight.end <- 51171641
...
credset.genes <- c("AL022328.1", "ALG12", "ARSA", "BRD1", "CHKB", "CHKB-CPT1B", 
                   "CPT1B", "DENND6B", "KLHDC7B", "LMF2", "MAPK8IP2", "MIOX", 
                   "MOV10L1", "NCAPH2", "ODF3B", "PLXNB2", "PPP6R2", "SELO", 
                   "SHANK3", "SYCE3", "TTLL8", "TUBGCP6", "TYMP", "ZBED4")
...
# Prep locus plot
pdf(paste(out.prefix, "locus_highlight.SHANK3.pdf", sep="."), height=3.75*2, width=3.25*2)
...
# Add PIPs
add.pips(pips, genes, y0=pip.panel.y0, panel.height=pip.panel.height, label.genes=c("SHANK3"),
...
add.genes(genes[which(genes$gene %in% credset.genes), ], n.rows=1, 
          ...
          y.axis.title="Credible Set", label.genes=c("SHANK3"))
add.genes(genes[which(!(genes$gene %in% credset.genes)), ], n.rows=1, 
...
legend.sides <- rep(c("left", "right"), 2)
