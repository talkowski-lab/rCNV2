...
hits.keep.idx <- which(scores$pHaplo >= hi.lc | scores$pTriplo >= ts.lc)
...
# hits.coefs <- gene.glm(scores[hits.keep.idx, ], feats,
#                        "hits.gradient", glm.alpha=NULL)
# hits.cor <- gene.cor(scores[hits.keep.idx, ], feats, "hits.gradient")
hits.coefs <- gene.glm(scores, feats, "hits.gradient", glm.alpha=NULL)
...
# hits.coefs.noConstr <- gene.glm(scores[hits.keep.idx, ], feats.noConstr,
#                                 "hits.gradient", glm.alpha=NULL)
# hits.cor.noConstr <- gene.cor(scores[hits.keep.idx, ], feats.noConstr, "hits.gradient")
hits.coefs.noConstr <- gene.glm(scores, feats.noConstr, "hits.gradient", glm.alpha=NULL)
...
sapply(list(list("ds", c(ns.color, cnv.colors[3]), c("gray30", cnv.blacks[3]),
       ...
       function(vals){
         ...
         pdf(paste(out.prefix, "gradient_regression.top_corr_coeffs", vals[[1]], "pdf", sep="."),
             height=pdf.dims[1], width=pdf.dims[2])
         plot.top.coefs(vals[[5]], feat.meta, colors=vals[[2]], borders=vals[[3]],
                        x.title=c(bquote("Spearman's" ~ rho)), N=coefs.to.plot)
         dev.off()
         # Excluding constraint features
