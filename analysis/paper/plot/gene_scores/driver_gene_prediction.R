  ...
  sapply(1:n.rows, function(ridx){
    sapply(1:n.cols, function(cidx){
      ...
      if(seg.idx <= length(plot.dat)){
        ...
        # sapply(1:n.cols, function(cidx){
        #   abline(v=seq(x.left[cidx], x.left[cidx]+1, 0.2), col="white")
        # })
...
print(n.segs.w.driver / nrow(segs))
n.dup.segs.w.driver <- length(which(sapply(seg.scores[which(names(seg.scores) %in% segs$region_id[which(segs$cnv=="DUP")])], function(df){any(df$score >= 0.9)})))
print(n.dup.segs.w.driver / length(which(segs$cnv=="DUP")))

# Compute fractions of segments with two or fewer high-confidence drivers
n.segs.w.lt2.drivers <- length(which(sapply(seg.scores, function(df){length(which(df$score >= 0.9)) %in% c(1, 2)})))
print(n.segs.w.lt2.drivers / nrow(segs))
