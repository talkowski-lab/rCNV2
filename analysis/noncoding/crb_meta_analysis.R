...
# Perform meta-analysis of CNV associations per cis-regulatory block (CRB)
...
read.stats <- function(stats.in, prefix, p.is.phred){
  ...
  cols.to.keep <- c("chr", "start", "end", "crb_id", "case_alt", "case_ref",
...
# Make meta-analysis data frame for a single CRB
...
# Perform meta-analysis for a single CRB
...
# Wrapper function to perform a meta-analysis on all crbs
meta <- function(stats.merged, cohorts, model="fe", saddle=T, secondary=T){
  ...
  meta.res <- meta.res[with(meta.res, order(chr, start, crb_id)), ]
  ...
  # Format output
  return(as.data.frame(cbind(meta.res[, which(colnames(meta.res) %in% c("chr", "start", "end", "crb_id",
...
# # Dev parameters
# setwd("~/scratch/")
# infile <- "~/scratch/HP0001250.rCNV.loose_noncoding.DEL.crb_burden.meta_analysis.input.txt"
# outfile <- "~/scratch/HP0001250.rCNV.loose_noncoding.DEL.crb_burden.meta_analysis.results.bed"
# corplot.out <- "~/scratch/HP0001250.rCNV.loose_noncoding.DEL.crb_burden.meta_analysis.or_corplot.png"
# model <- "fe"
# p.is.phred <- T
# spa <- T
# secondary <- T
